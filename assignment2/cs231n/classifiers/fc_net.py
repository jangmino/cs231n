...
def affine_bn_relu_forward(x, w, b, gamma=None, beta=None, bn_param=None, dropout_param=None):
    """
...
def affine_bn_relu_backward(dout, cache, bn_param=None, dropout_param=None):
    """
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss += 0.5 * self.reg *( np.sum(cache1[0][1] * cache1[0][1])
                                # + np.sum(cache1[0][2] * cache1[0][2])
                                + np.sum(cache2[1] * cache2[1])
                                #+ np.sum(cache2[2] * cache2[2])
                                   )

        # the regularization is only applied to W1 and W2 not b1 and b2 (cs231n's implementation)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, h in enumerate(hidden_dims):
            self.params['W%d' % (i+1)] = weight_scale * np.random.randn(prev_dim, h)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            dic_out[i + 1], dic_cache[i + 1] = affine_bn_relu_forward(dic_out[i], self.params['W%d' % (i + 1)],
                                                                      self.params['b%d' % (i + 1)],
                                                                      self.params['gamma%d' % (i + 1)] if self.use_batchnorm else None,
                                                                      self.params['beta%d' % (i + 1)] if self.use_batchnorm else None,
                                                                      self.bn_params[i] if self.use_batchnorm else None,
                                                                      self.dropout_param if self.use_dropout else None
                                                                      )

        dic_out[self.num_layers], dic_cache[self.num_layers] = affine_forward(dic_out[self.num_layers - 1],
                                                                              self.params['W%d' % self.num_layers],
                                                                              self.params['b%d' % self.num_layers]
                                                                              )

        scores = dic_out[self.num_layers]
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dX[self.num_layers], grads['W%d'%self.num_layers], grads['b%d'%self.num_layers] = affine_backward(dscores, dic_cache[self.num_layers])
        ...
        for i in range(self.num_layers - 1, 0, -1):
            #print('i', i)
            #print('dX', dX[3])
            #print('dic_cache', dic_cache[2])
            #print('bn_params', self.bn_params)
            #print('dropout_param', self.dropout_param)
            dx, dw, db, dgamma, dbeta = \
                affine_bn_relu_backward(dX[i + 1], dic_cache[i],
                                        self.bn_params[i-1] if self.use_batchnorm else None,
                                        self.dropout_param if self.use_dropout else None
                                        )
            loss += 0.5 * self.reg * np.sum(self.params['W%d' % i] * self.params['W%d' % i])
            dX[i], grads['W%d' % i], grads['b%d' % i] = dx, dw, db
            if self.use_batchnorm:
